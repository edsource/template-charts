...
function chart_shortcode($attr){
	$attr = shortcode_atts(array('class'=>null, 'type'=>null, 'data'=>null, 'contain'=>null,'width'=>450,'height'=>300, 'color'=>'BuPu', 'm'=>null,'items'=>null, 'sort'=>false, 'format'=>null, 'ticks'=>5,'padding'=>.1, 'ylabel'=>null, 'xlabel'=>null, 'title'=>null, 'subhed'=>null,'source'=>null, 'max'=>null, 'inrad'=>null, 'legend'=>null), $attr, 'chart');
	/* IF CHART NULL, ESCAPE */
	if ($attr['type'] == null || $attr['contain'] == null || $attr['data'] == null ){return;}

	/* MARGIN MAGIC */
	if ($attr['m'] == null){
		$attr['margin'] = [50,40,30,50];
	}
	else {
	...
	switch($attr['type']){
		case 'stackedbarpercent':
			$output .= '<div class="temp-charts" role="stackedbarpercent" id="'.$attr['contain'].'"></div>';
		...
		case 'pie':
			...
			$output .= '<div id="'.$attr['contain'].'"></div>';
